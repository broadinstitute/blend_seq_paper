 ...
 *   (needed by Paragraph).
 */
public class CleanVCFParagraph {
    /**
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (str!=null) {
            ...
            // Ensuring the right END (needed by Paragraph).
            p=tokens[7].indexOf(END_STR);
            if (p>=0) {
                q=tokens[7].indexOf(";",p+1);
                if (q>=0) tokens[7]=tokens[7].substring(0,p)+tokens[7].substring(q+1);
                else tokens[7]=tokens[7].substring(0,p);
            }                
            if (row==0 || row==1 || row==2) {
                // DEL, INV, DUP.
                tmpString=getField(tokens[7],SVLEN_STR);
            ...
            // Ensuring sequence REF for every DEL (symbolic and non-symbolic).
            // Non-symbolic DELs need to be fixed because Sniffles may not use
